...
def labels_processor(labels, img_size, num_classes):
  ...
  for b in range(labels.shape[0]):
    for i in range(img_size):
      for j in range(img_size):
        if labels[b, i, j] != 0:
          print("labels_processor:> Labels[%d, %d, %d]: %d" % (b, i, j, labels[b, i, j]))
          labels_after[b, labels[b, i, j] - 1] += 1
  return labels_after

def evaluate_result(outputs, labels, eps = 1e-4):
  assert(len(outputs.shape) == 2) # (batch_size, num_classes)
  assert(outputs.shape == labels.shape)
  for b in range(outputs.shape[0]):
    for c in range(outputs.shape[1]):
      if labels[b, c] > eps and outputs[b, c] < eps:
         return 0 # class losing
  return 1