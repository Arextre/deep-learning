...
class ReLUActivator(object):
  ...
  def backward(self, output):
    if type(output) == float:
      ...
      return np.array([1.0 if ele > 0 else 0.0 for ele in output]).reshape(output.shape)
...
class Network(object):
  def __init__(self, layers, lossfunc = 'CrossEntropyLoss'):
    ...
    self.layers = []
    self.layers.append(FullConnectedLayer(layers[0], layers[1], 'SigmoidActivator'))
  ...
  def calc_grad(self, label):
    ...
    """
    delta = self.layers[-1].output * np.sum(label) - label
...
def grad_check_test():
  net = Network([6, 10, 2])
  sample_feature = np.array([[0.38], [0.998], [0.1], [0.33], [0.42], [1.3]])
